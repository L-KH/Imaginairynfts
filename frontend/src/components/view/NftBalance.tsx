...
function NFTlist() {
  ...
  const [listingNFT, setListingNFT] = useState<IData | null>(null);
  ...
  const openListingModal = (nft: IData) => {
    setListingNFT(nft);
...
<button 
  className="text-gray-500 hover:text-sky-400 dark:hover:text-white ms-5 inset-y-0 right-0"
  onClick={() => openListingModal(item)}
>
  <span className="text-xs pr-4">List in Marketplace</span>
</button>
...
const ListingModal = ({ nft, isOpen, onClose }: { nft: IData | null, isOpen: boolean, onClose: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsListing(true);
    try {
      await handleListNFT(BigInt(nft.id), price);
